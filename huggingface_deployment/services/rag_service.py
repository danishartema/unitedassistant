        ...
        try:
            ...
            for phase in previous_phases[-3:]:  # Last 3 phases for immediate context
                if phase.ai_response:
                    context_parts.append(f"Phase {phase.phase_number} ({phase.title}):{chr(10)}{phase.ai_response}")
            ...
            for phase, similarity in similar_phases:
                if phase.phase_number not in added_phases and phase.ai_response:
                    context_parts.append(
                        f"Related content from Phase {phase.phase_number} ({phase.title}) [similarity: {similarity:.2f}]:{chr(10)}{phase.ai_response}"
                    )
