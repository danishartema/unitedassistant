...
Format the response as a professional report with clear sections and actionable insights."""
    ...
    async def generate_combined_summary(self, completed_modules: dict) -> str:
        """Generate a combined summary for all completed modules with rate limiting."""
        try:
            combined_sections = []
            module_count = 0
            
            for module_id, answers in completed_modules.items():
                if module_id in self.modules:
                    module_count += 1
                    logger.info(f"Processing module {module_count}/{len(completed_modules)}: {module_id}")
                    
                    # Generate individual module summary with rate limiting
                    module_summary = await self.generate_module_summary(module_id, answers)
                    combined_sections.append(f"## {module_summary['module_name']}\n\n{module_summary['summary']}")
                    
                    # Add delay between API calls to avoid rate limiting
                    if module_count < len(completed_modules):
                        await asyncio.sleep(2)  # 2 second delay between modules
            
            # Create overall summary
            overall_summary = f"""# Complete Project Summary

This comprehensive report combines insights from all completed modules:

{chr(10).join(combined_sections)}

## Overall Assessment

Based on the responses across all modules, here are the key insights and recommendations for your project.

**Total Modules Completed:** {len(completed_modules)}
**Key Areas Covered:** {', '.join([self.modules.get(mid, {}).get('name', mid) for mid in completed_modules.keys() if mid in self.modules])}

---
*Generated by Unified Assistant - Comprehensive Analysis*
"""
